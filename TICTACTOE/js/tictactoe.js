...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //active player may only be 'X' or 'O' so, if not 'X' it must be 'O'    
            ...
            //if activePlayer is equal to 'O', the o.ng is placed in HTML
        ...
        if(activePlayer ==='X') {
            //if cative player is 'X' change it to 'O'
            activePlayer= 'O';
    ...
    function computersTurn() {
        ...
        while(!success) {
            ...
            if(placeXOrO(pickASquare)) {
                ...
                //this changes our boolean and ends hte loop
                success=true;
            };
        }
    }
...
function checkWinConditions() {
    ...
    //this condition checks for tie. if none of hte above conditions register
    //and 9 squares are selected, the code executes
    else if (selectedSquares.length >=9) {
        //this function plays the tie game sound
        audio('media2/TieGame2.mp3');
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y2 = coordY2,
        //this variable stores temporary x axis data wwe update in our animation loop
        x = x1,
        //this variable stores temporary y asxis data we update in pur animation loop
        y = y1;
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            ...
            if (x < x2) {x += 10;}
            //this condition adds 10 to our previous end y point
            if (y < y2) {y += 10;}
            //this condition cancels our animation loop if reach the end points
            if (x>= x2 && y >= y2) {cancelAnimationFrame(animationLoop);}
        }
        //this condition is similar to the one above
        //it was necessary for hte 6, 4, 2 win condition
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) {x += 10;}
            if (y > y2) {y -=10;}
            if (x >= x2 && y <= y2) {cancelAnimationFrame(animationLoop);}
    ...
    //this line dsallows clicking while the win sound is playing
    disableClick();
    //this line plays the win sounds
    audio('./media2/WinGame2.mp3');
